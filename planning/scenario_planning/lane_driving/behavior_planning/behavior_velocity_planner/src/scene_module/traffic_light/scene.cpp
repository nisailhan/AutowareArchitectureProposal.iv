...
bool createTargetPoint(
  const autoware_planning_msgs::PathWithLaneId & input, const LineString2d & stop_line,
...
autoware_perception_msgs::LookingTrafficLightState initializeTrafficLightState(
  const ros::Time stamp)
{
  autoware_perception_msgs::LookingTrafficLightState state;
  ...
  if (state_ == State::APPROACH) {
    ...
    if (use_initialization_after_start) {
      if (signed_arc_length_to_stop_point > restart_length) {
        ROS_INFO("GO_OUT(RESTART) -> APPROACH");
        state_ = State::APPROACH;
      }
    }
    return true;
...
bool TrafficLightModule::isStopSignal(const lanelet::ConstLineStringsOrPolygons3d & traffic_lights)
{
  ...
  return looking_tl_state_.final.judge ==
         autoware_perception_msgs::TrafficLightStateWithJudge::STOP;
}
...
bool TrafficLightModule::isPassthrough(const double & signed_arc_length) const
{
  ...
  // Calculate distance until ego vehicle decide not to stop, taking into account the jerk and acceleration.
  ...
  if (enable_pass_judge && !stoppable && !is_prev_state_stop_ && input_ == Input::PERCEPTION) {
    ...
    if (!reachable) {
      ...
      // pass through
      ROS_WARN_THROTTLE(1.0, "[traffic_light] can pass through intersection during yellow lamp");
      return true;
    }
  } else {
    return false;
...
{
  if (hasLampState(tl_state, autoware_perception_msgs::LampState::GREEN)) {
    return false;
  }
  ...
  if (
    ...
    hasLampState(tl_state, autoware_perception_msgs::LampState::RIGHT)) {
    return false;
  }
  if (
    turn_direction == "left" && hasLampState(tl_state, autoware_perception_msgs::LampState::LEFT)) {
    return false;
  }
  if (
    turn_direction == "straight" &&
    hasLampState(tl_state, autoware_perception_msgs::LampState::UP)) {
    return false;
  }
  ...
  autoware_perception_msgs::TrafficLightStateStamped & highest_confidence_tl_state)
{
  ...
  for (const auto & traffic_light : traffic_lights) {
    ...
    ROS_DEBUG("traffic light id:%d (on route)", id);
...
autoware_planning_msgs::PathWithLaneId TrafficLightModule::insertStopPose(
  ...
  const Eigen::Vector2d & target_point, autoware_planning_msgs::StopReason * stop_reason)
{
  ...
  stop_factor.stop_factor_points =
    std::vector<geometry_msgs::Point>{debug_data_.highest_confidence_traffic_light_point};
...
TrafficLightModule::generateTlStateWithJudgeFromTlState(
  const autoware_perception_msgs::TrafficLightState tl_state) const
{
  autoware_perception_msgs::TrafficLightStateWithJudge tl_state_with_judge;
  ...
  tl_state_with_judge.judge = isTrafficLightStateStop(tl_state)
                                ? autoware_perception_msgs::TrafficLightStateWithJudge::STOP
                                : autoware_perception_msgs::TrafficLightStateWithJudge::GO;
