...
geometry_msgs::Pose calcInterpolatedPose(
  ...
  const StopLineModule::SegmentIndexWithOffset & offset_segment)
{
  ...
  const auto interpolated_point_2d = p_eigen_front + offset_segment.offset * direction_vector;
  ...
  const autoware_planning_msgs::PathWithLaneId & path, const geometry_msgs::Point & point)
{
  ...
  std::transform(
    ...
    [&point](const autoware_planning_msgs::PathPointWithLaneId & p) {
...
double calcSignedArcLength(
  const autoware_planning_msgs::PathWithLaneId & path, const size_t & idx_front,
...
{
  const auto base_backward_length = planner_param_.stop_margin + planner_data_->base_link2front;
...
autoware_planning_msgs::PathWithLaneId StopLineModule::insertStopPose(
  const autoware_planning_msgs::PathWithLaneId & path,
