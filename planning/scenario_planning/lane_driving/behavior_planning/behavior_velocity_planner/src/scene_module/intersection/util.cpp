  ...
  int * pass_judge_line_idx, int * first_idx_inside_lane)
{
  ...
  if (getStopPoseIndexFromMap(path_ip, lane_id, planner_data, stop_idx_ip, 10.0)) {
  ...
  const std::shared_ptr<const PlannerData> & planner_data, int & stop_idx_ip, int dist_thr)
{
  ...
  for (size_t i = 0; i < path.points.size() - 1; i++) {
    ...
    ROS_DEBUG("[Intersection Util] found collision point");
  ...
  if (!planning_utils::calcClosestIndex(path, stop_point_from_map, stop_idx_ip, dist_thr)) {
    ROS_DEBUG("[Intersection Util] found stop line, but not found stop index");
    return false;
  }

  ROS_DEBUG("[Intersection Util] found stop line and stop index");
  ...
  std::vector<lanelet::CompoundPolygon3d> * objective_polygons)
{
  ...
  *objective_polygons = getPolygon3dFromLaneletsVec(objective_lanelets_sequences, length);
  ...
  for (const auto l : objective_lanelets) ss_o << l.front().id() << ", ";
