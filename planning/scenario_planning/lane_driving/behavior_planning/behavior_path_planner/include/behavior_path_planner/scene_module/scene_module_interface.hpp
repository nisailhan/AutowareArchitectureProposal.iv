...
#ifndef BEHAVIOR_PATH_PLANNER_SCENE_MODULE_INTERFACE_HPP
#define BEHAVIOR_PATH_PLANNER_SCENE_MODULE_INTERFACE_HPP
...
{
using PlanResult = std::shared_ptr<autoware_planning_msgs::PathWithLaneId>;

struct TurnSignalInfo
{
  TurnSignalInfo() { turn_signal.data = autoware_vehicle_msgs::TurnSignal::NONE; }
...
{
public:
  explicit SceneModuleInterface(const std::string & name) : name_(name) {}
  ...
  virtual BehaviorModuleOutput planWaitingApproval()
  {
    ...
    out.path_candidate = std::make_shared<autoware_planning_msgs::PathWithLaneId>(planCandidate());
    return out;
  }
   ...
   */
  virtual autoware_planning_msgs::PathWithLaneId planCandidate() const = 0;
  ...
  virtual BehaviorModuleOutput run()
  {
    ...
    if (approval_handler_.isApproved()) {
      ROS_DEBUG("Was waiting approval, and now approved. Do plan().");
      approval_handler_.clearWaitApproval();
      return plan();
    } else {
      ROS_DEBUG("keep waiting approval... Do planCandidate().");
