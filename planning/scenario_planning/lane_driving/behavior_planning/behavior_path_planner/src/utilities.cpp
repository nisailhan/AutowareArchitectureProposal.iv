  ...
  const lanelet::ConstLanelets & target_lanelets)
{
  ...
  for (size_t i = 0; i < objects.objects.size(); i++) {
    ...
    Polygon2d obj_polygon;
    if (!calcObjectPolygon(obj, &obj_polygon)) {
      ROS_ERROR_STREAM(__func__ << ": Failed to calcObjectPolygon...!!!");
      continue;
    }
...
std::vector<double> calcObjectsDistanceToPath(
  const autoware_perception_msgs::DynamicObjectArray & objects,
  const autoware_planning_msgs::PathWithLaneId & ego_path)
{
  ...
  const RouteHandler & route_handler)
{
  ...
  // occupancy_grid.data = image;
  {
    ...
    // Closing
    constexpr int num_iter = 2;  // TODO(Horibe) Think later.
...
{
  auto centerline_path = std::make_shared<autoware_planning_msgs::PathWithLaneId>();
...
{
  autoware_perception_msgs::DynamicObjectArray filtered;
