...
void BehaviorTreeManager::createBehaviorTree()
{
  ...
  try {
    ...
    ROS_ERROR("Failed to create BT from: %s", bt_manager_param_.bt_tree_config_path.c_str());
    exit(EXIT_FAILURE);
  }
  addGrootMonitoring(
...
void BehaviorTreeManager::registerSceneModule(const std::shared_ptr<SceneModuleInterface> & module)
{
  ...
  bt_factory_.registerSimpleCondition(name + "_Request", [this, module, status](BT::TreeNode &) {
    return isExecutionRequested(module, status);
  });
  bt_factory_.registerSimpleCondition(name + "_Ready", [this, module, status](BT::TreeNode &) {
...
BehaviorModuleOutput BehaviorTreeManager::run(const std::shared_ptr<PlannerData> & data)
{
  ...
  ROS_DEBUG("BehaviorPathPlanner::run end status = %s", BT::toStr(res).c_str());
...
BT::NodeStatus BehaviorTreeManager::checkForceApproval(const std::string & name)
{
  ...
  if ((ros::Time::now() - approval.stamp).toSec() > 1.0) {
    ROS_WARN_THROTTLE(3.0, "BehaviorTreeManager : Force approval data is time out!");
