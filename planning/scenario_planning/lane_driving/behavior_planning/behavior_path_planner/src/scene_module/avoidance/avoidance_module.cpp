...
lanelet::ConstLanelets calcLaneAroundPose(
  const std::shared_ptr<const PlannerData> & planner_data, const geometry_msgs::Pose & pose,
...
{
AvoidanceModule::AvoidanceModule(const std::string & name, const AvoidanceParameters & parameters)
...
{
  ROS_DEBUG("AVOIDANCE isExecutionRequested");
...
{
  ROS_DEBUG("AVOIDANCE isExecutionReady");
...
AvoidancePlanningData AvoidanceModule::calcAvoidancePlanningData() const
{
  ...
  ROS_DEBUG("dynamic_objects.size() = %lu", planner_data_->dynamic_object->objects.size());
  ...
  for (const auto & i : lane_filtered_objects_index) {
    ...
    if (!isTargetObjectType(object)) {
      ROS_DEBUG("isTargetObjectType false. Ignore this object.");
      continue;
    }
    ...
    if (object_data.longitudinal < -parameters_.object_check_backward_distance) {
      ROS_DEBUG("object < -backward_distance. Ignore this object.");
      continue;
    }
    ...
    if (std::abs(object_data.lateral) < parameters_.threshold_distance_object_is_on_center) {
      ROS_DEBUG("Object is on Center. Ignore this object.");
      continue;
    }
  ...
  ROS_DEBUG("target object size = %lu", data.objects.size());
...
std::vector<ShiftPoint> AvoidanceModule::calcShiftPointsInFrenet(DebugData & debug) const
{
  ...
  std::sort(sorted_objects.begin(), sorted_objects.end(), [](auto a, auto b) {
  ...
  for (const auto & sl : shift_lengths) {
    ROS_DEBUG("[RAW SHIFT LENGTH] lon = %f, lat = %f", sl.longitudinal, sl.lateral);
  }
  ...
  for (const auto & sl : shift_lengths) {
    ROS_DEBUG("[MARGINED SHIFT LENGTH] lon = %f, lat = %f", sl.longitudinal, sl.lateral);
  }
  ...
  if (shift_lengths.empty() && std::fabs(path_shifter_.getLastShiftLength()) > 0.01) {
    ...
    ROS_DEBUG("add return point: to_start = %f, to_end = %f", dist_to_start, dist_to_end);
  ...
  for (size_t i = 0; i < shift_lengths.size(); ++i) {
    ...
    if (std::abs(delta_shift) < 0.5) {
      ROS_DEBUG("delta_shift = %f, it is small. Do NOT add this point.", delta_shift);
      continue;
    }
  ...
  ROS_DEBUG("shift_points size = %lu", shift_points.size());
  ...
  if (!shift_lengths.empty()) {  // if empty, the return-shift point was taken care of in above.
    if (std::abs(shift_lengths.back().lateral) > 0.01) {  // take back, assuming it is sorted.
      const auto dist_ego_to_pathend = autoware_utils::calcSignedArcLength(
        reference.points, getEgoPosition(), reference.points.size() - 1);

      // TODO(Horibe) think the length later
      if (dist_ego_to_pathend - shift_lengths.back().longitudinal > 50.0) {
  ...
  if (use_intersection) {
    ...
    if (intersection_shift_point) {
      ROS_ERROR("ADD INTERSECTION point!!!");
      shift_points.push_back(*intersection_shift_point);
    } else {
      ROS_ERROR("NO intersection point found");
    }
  }
...
geometry_msgs::PoseStamped AvoidanceModule::getUnshiftedEgoPose(const ShiftedPath & prev_path) const
{
  ...
  geometry_msgs::PoseStamped unshifted_pose;
  ...
  const geometry_msgs::PoseStamped & pose) const
{
  ...
  //     centerline_path.points, centerline_path.points.front().point.pose.position, getEgoPosition());
  //   ROS_INFO("actual back_to_ego distance = %f", back_to_ego);
  // }
  ...
  const AvoidancePlanningData & data) const
{
  ...
  for (const auto & p : avoidance_data_.reference_path.points) {
    for (const auto & id : p.lane_ids) {
      ...
      if (turn_direction == "right" || turn_direction == "left") {
        ...
        ROS_INFO("intersection is found.");
        break;
      }
    }
  ...
  const auto calcBehindPose = [&data](const geometry_msgs::Point & p, const double dist) {
  ...
  const auto intersection_shift_point = [&]() {
    ...
    if (ego_to_intersection_dist <= 5.0) {
      ROS_INFO("No enough margin to intersection.");
      return shift_point;
    }
    ...
    for (const auto & obj : avoidance_data_.objects) {
      if (std::abs(obj.longitudinal - ego_to_intersection_dist) > intersection_obstacle_check_dist)
        continue;
      if (isOnRight(obj)) continue;  // TODO(Horibe) Now only think about the left side obstacle.
      shift_length = std::min(shift_length, obj.overhang_dist - intersection_shift_margin);
    }
    ROS_INFO("Intersection shift_length = %f", shift_length);
...
BehaviorModuleOutput AvoidanceModule::plan()
{
  ...
  ROS_DEBUG("exit plan(): set prev output (back().lat = %f)", prev_output_->shift_length.back());
...
{
  ROS_DEBUG("AVOIDANCE planCandidate start");
...
void AvoidanceModule::addShiftPointIfApproved(const ShiftPoint & shift_point)
{
  if (approval_handler_.isApproved()) {
    ROS_DEBUG("We want to add this shift point, and approved. ADD SHIFT POINT!");
    const size_t prev_size = path_shifter_.getShiftPointsSize();
    path_shifter_.addShiftPoint(shift_point);
    ROS_DEBUG("shift_point size: %lu -> %lu", prev_size, path_shifter_.getShiftPointsSize());

    // use this approval.
    approval_handler_.clearApproval();  // TODO(Horibe) will be fixed with service-call?
  } else {
    ROS_DEBUG("We want to add this shift point, but NOT approved. waiting...");
    approval_handler_.waitApproval();
  }
}
...
{
  if (points.empty()) {
    ROS_DEBUG("shift points candidate is empty. return None.");
    return {};
  }

  if (shifter.getShiftPoints().empty()) {
    ROS_DEBUG("current shift point is empty. The first point should be NEW one.");
    return points.front();
  }
  ...
  for (const auto & point_candidate : points) {
    ...
    for (const auto & current_point : shifter.getShiftPoints()) {
      ...
      if (shift_length_diff < shift_threshold_m && std::abs(dist) < longitudinal_threshold_m) {
        ROS_DEBUG("A similar shift point found. This is NOT new.");
    ...
    if (we_should_add_this) {
      ROS_DEBUG("No similar shift point found. This is NEW!");
      return point_candidate;
    }
  }

  ROS_DEBUG("No new shift point found. return None.");
  return {};
}
...
{
  ROS_DEBUG("AVOIDANCE onEntry. wait approval!");
  initVariables();
  current_state_ = BT::NodeStatus::SUCCESS;
  approval_handler_.waitApproval();
  return;
...
{
  using autoware_perception_msgs::Semantic;
...
void AvoidanceModule::setDebugData(const PathShifter & shifter, const DebugData & debug)
{
  ...
  const auto add = [this](const visualization_msgs::MarkerArray & added) {
  ...
  add(createFrenetPointMarkerArray(
  ...
  if (!path.shift_length.empty()) {
    if (isAvoidancePlanRunning()) {
      ...
      if (diff > tl_on_threshold) {
        turn_signal.turn_signal.data = autoware_vehicle_msgs::TurnSignal::LEFT;
      } else if (diff < -tl_on_threshold) {
        turn_signal.turn_signal.data = autoware_vehicle_msgs::TurnSignal::RIGHT;
      }
    }
  }
