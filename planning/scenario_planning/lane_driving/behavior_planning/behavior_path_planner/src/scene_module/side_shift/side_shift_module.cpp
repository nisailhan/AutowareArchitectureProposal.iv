...
{
SideShiftModule::SideShiftModule(const std::string & name, const SideShiftParameters & parameters)
: SceneModuleInterface(name), pnh_("~"), parameters_(parameters)
{
  initVariables();

  lateral_offset_subscriber_ =
    pnh_.subscribe("input/side_shift_length", 1, &SideShiftModule::onLateralOffset, this);
}
...
bool SideShiftModule::isExecutionRequested() const
{
  ...
  ROS_DEBUG("ESS::isExecutionRequested() : %s!!", has_request ? "TRUE" : "FALSE");
...
BT::NodeStatus SideShiftModule::updateState()
{
  ...
  ROS_DEBUG("ESS::updateState() : no_request = %d, on_reference = %d", no_request, on_reference);
...
void SideShiftModule::onLateralOffset(const std_msgs::Float32::ConstPtr & lateral_offset_msg)
{
  const double new_lateral_offset = lateral_offset_msg->data;
...
PathWithLaneId SideShiftModule::refinePath(
  ...
  geometry_msgs::Pose & start_avoid_pose) const
{
...
bool SideShiftModule::isVehicleInDrivableArea(const nav_msgs::OccupancyGrid & drivable_area) const
{
  // Get grid origin
  ...
  cv::Point cv_front_left, cv_front_right, cv_rear_left, cv_rear_right;
  {
    ...
    auto toCVPointLocal = [&](const geometry_msgs::Point & p) {
