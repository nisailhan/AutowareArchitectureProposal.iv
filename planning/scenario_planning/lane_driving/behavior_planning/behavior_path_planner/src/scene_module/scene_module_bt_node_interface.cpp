  ...
  const std::shared_ptr<SceneModuleStatus> & status)
{
  ...
  ROS_DEBUG_STREAM(__func__ << ": name = " << p->name() << ", result = " << ret);
...
{
  ROS_DEBUG_STREAM("bt::tick is called. module name: " << scene_module_->name());
  ...
  if (is_waiting_approval) {
    try {
      ...
      if (!res) ROS_ERROR_STREAM("setOutput() failed : " << res.error());
  ...
  while (ros::ok()) {
    ...
    if (current_status != BT::NodeStatus::RUNNING) {
      ROS_DEBUG("on tick: module ended.");
      break;
    }
  ...
  ROS_DEBUG_STREAM("on tick: return current status = " << BT::toStr(current_status));
...
{
  ROS_DEBUG_STREAM("bt::planCandidate module name: " << scene_module_->name());
