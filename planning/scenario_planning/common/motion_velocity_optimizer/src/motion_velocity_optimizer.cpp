  ...
  int& current_pose_round_idx, double& length_to_round_waypoint) const
{
  ...
  bool before_traj = vpu::calcWhichSideOfLine(traj->points.at(1).pose.position, traj->points.at(0).pose.position, current_pose_ptr_->pose.position);
  bool after_traj = not vpu::calcWhichSideOfLine(traj->points.at(traj->points.size() - 1).pose.position, traj->points.at(traj->points.size() - 2).pose.position, current_pose_ptr_->pose.position);

  if (before_traj) {
    current_pose_round_idx = 0;
    length_to_round_waypoint = -vpu::calcTriangleVerticalInterpolatedLength(current_pose_ptr_->pose.position, traj->points.at(0).pose.position, traj->points.at(1).pose.position);
  } else if (after_traj) {
    current_pose_round_idx = traj->points.size() - 1;
    length_to_round_waypoint = -vpu::calcTriangleVerticalInterpolatedLength(current_pose_ptr_->pose.position, traj->points.at(traj->points.size() - 1).pose.position, traj->points.at(traj->points.size() - 2).pose.position);
    ...
    if (stop_idx == 0) {
      ...
      length_to_round_waypoint = vpu::calcTriangleVerticalInterpolatedLength(current_pose_ptr_->pose.position, traj->points.at(1).pose.position, traj->points.at(0).pose.position);
    }
    ...
    if (stop_idx == static_cast<int>(traj->points.size() - 1)) {
      ...
      length_to_round_waypoint = vpu::calcTriangleVerticalInterpolatedLength(current_pose_ptr_->pose.position, traj->points.at(traj->points.size() - 2).pose.position, traj->points.at(traj->points.size() - 1).pose.position);
    } else {
      current_pose_round_idx = traj->points.size() - 1;
      length_to_round_waypoint = vpu::calcTriangleVerticalInterpolatedLength(current_pose_ptr_->pose.position, traj->points.at(traj->points.size() - 1).pose.position, traj->points.at(traj->points.size() - 2).pose.position);
    }
  } else {
    double prev_dist = vpu::calcDist2d(current_pose_ptr_->pose, traj->points.at(current_pose_idx - 1).pose);
    double next_dist = vpu::calcDist2d(current_pose_ptr_->pose, traj->points.at(current_pose_idx + 1).pose);

    if (before_stopline) {
      if (prev_dist <= next_dist) {
        current_pose_round_idx = current_pose_idx;
        length_to_round_waypoint = vpu::calcTriangleVerticalInterpolatedLength(current_pose_ptr_->pose.position, traj->points.at(current_pose_idx).pose.position, traj->points.at(current_pose_idx - 1).pose.position);
      } else {
        current_pose_round_idx = current_pose_idx + 1;
        length_to_round_waypoint = vpu::calcTriangleVerticalInterpolatedLength(current_pose_ptr_->pose.position, traj->points.at(current_pose_idx + 1).pose.position, traj->points.at(current_pose_idx).pose.position);
      }
    } else {
      if (prev_dist <= next_dist) {
        current_pose_round_idx = current_pose_idx - 1;
...
{
  autoware_planning_msgs::Trajectory::ConstPtr traj = base_traj_raw_ptr_;
  ...
  if (current_pose_idx < 0) {
    ROS_WARN_THROTTLE(5.0, "[publish stop distance] cannot find closest waypoint for input trajectory");
    return;
  }
  ...
  if (traj->points.size() == 1) {
    ROS_WARN_THROTTLE(5.0, "[publish stop distance] trajectory size is 1");
    return;
  }
  ...
  calcRoundWaypointFromCurrentPose(traj, before_stopline, current_pose_idx, stop_idx, current_pose_round_idx, length_to_round_waypoint);
  ...
  dist_to_stopline.data = stop_dist;
