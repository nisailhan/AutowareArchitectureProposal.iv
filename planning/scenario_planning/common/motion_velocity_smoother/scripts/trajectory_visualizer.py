...
class TrajectoryVisualizer():
    ...
    def __init__(self):
        ...
        self.substatus1 = message_filters.Subscriber("/planning/scenario_planning/motion_velocity_smoother/debug/trajectory_external_velocity_limited",
                                           Trajectory, queue_size=1, buff_size=BUFFER_SIZE, tcp_nodelay=True)
        self.substatus2 = message_filters.Subscriber("/planning/scenario_planning/motion_velocity_smoother/debug/trajectory_lateral_acc_filtered",
                                           Trajectory, queue_size=1, buff_size=BUFFER_SIZE, tcp_nodelay=True)
        self.substatus3 = message_filters.Subscriber("/planning/scenario_planning/motion_velocity_smoother/debug/trajectory_raw",
                                           Trajectory, queue_size=1, buff_size=BUFFER_SIZE, tcp_nodelay=True)
        self.substatus4 = message_filters.Subscriber("/planning/scenario_planning/motion_velocity_smoother/debug/trajectory_time_resampled",
                                           Trajectory, queue_size=1, buff_size=BUFFER_SIZE, tcp_nodelay=True)
    ...
    def plotTrajectory(self, data):
        ...
        if self.update_traj_final:
            ...
            if len(y) != 0:
                ...
                self.ax3.set_ylim([-1.0, 1.0])  # fixed range
                if len(x) == len(y):
                    self.im4.set_data(x, y)
