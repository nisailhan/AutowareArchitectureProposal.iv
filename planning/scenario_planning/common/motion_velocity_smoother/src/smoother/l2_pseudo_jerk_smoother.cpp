  ...
  const autoware_planning_msgs::Trajectory & input, autoware_planning_msgs::Trajectory & output)
{
  ...
  if (std::fabs(input.points.front().twist.linear.x) < 0.1) {
    ROS_DEBUG("[L2PseudoJerkSmoother] closest v_max < 0.1. assume vehicle stopped. return.");
    return false;
  }
  ...
  if (N < 2) {
    ROS_WARN("[L2PseudoJerkSmoother] trajectory length is not enough.");
    return false;
  }
  ...
  */
  ...
  for (unsigned int i = 0; i < N; ++i) {
    ...
    //std::cout << "[smoother] v[" << i << "] : " << std::sqrt(std::max(v, 0.0)) << ", v_max[" << i << "] : " << v_max[i] << std::endl;
  ...
  if (status_val != 1) {
    ROS_WARN(
      "[L2PseudoJerkSmoother] optimization failed : %s", qp_solver_.getStatusMessage().c_str());
  }
  ...
  ROS_DEBUG(
    "[L2PseudoJerkSmoother] init time = %f [ms], optimization time = %f [ms]", dt_ms1, dt_ms2);
