...
class CarModel(object):
    ...
    def _get_four_points(self):
        ...
        left = 0.5 * self.width
        P = np.array([[back, left], [back, right], [
...
def create_costmap_msg():
    ...
    data = np.zeros((info.height, info.width))
    data[:, :10] = 100.0
    data[:, -10:] = 100.0
    data[:10, :] = 100.0
    data[-10:, :] = 100.0
...
class Plotter(object):
    def __init__(self, msg):
        ...
        tmp = np.array(msg.data).reshape(
            (n_grid[1], n_grid[0]))  # about to be transposed!!
    ...
    def plot(self,
             ...
             plan_duration=None):
        fig, ax = plt.subplots()
        xlin, ylin = [np.linspace(
            self.b_min[i], self.b_max[i], self.n_grid[i]) for i in range(2)]
        ...
        ax.axis("equal")
        ax.text(4, 25, "elapsed : {0} [sec]".format(
            plan_duration*1e-3), fontsize=12)
...
if __name__ == '__main__':
    for postfix in ["single", "multi"]:
        for idx in range(4):
            pose_list = []
            filebase = "/tmp/result_{0}{1}".format(postfix, idx)
            with open(filebase + ".txt", "r") as f:
                reader = csv.reader(f)
