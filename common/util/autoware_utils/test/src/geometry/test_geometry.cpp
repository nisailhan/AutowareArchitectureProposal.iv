...
TEST(geometry, getPose)
{
...

  {
    ...
    const geometry_msgs::Pose p_out = getPose(p);
...
TEST(geometry, transform2pose)
{
...

  {
    ...
    const geometry_msgs::Pose pose = transform2pose(transform);
...

  {
    ...
    transform_stamped.header.seq = 1;
    transform_stamped.header.stamp = ros::Time(2.0);
    ...
    const geometry_msgs::PoseStamped pose_stamped = transform2pose(transform_stamped);

    EXPECT_EQ(transform_stamped.header.frame_id, pose_stamped.header.frame_id);
    EXPECT_EQ(transform_stamped.header.seq, pose_stamped.header.seq);
    EXPECT_DOUBLE_EQ(transform_stamped.header.stamp.toSec(), pose_stamped.header.stamp.toSec());
...
TEST(geometry, pose2transform)
{
...

  {
    ...
    const geometry_msgs::Transform transform = pose2transform(pose);
...

  {
    ...
    pose_stamped.header.stamp = ros::Time(2.0);
    ...
    const geometry_msgs::TransformStamped transform_stamped = pose2transform(pose_stamped);

    EXPECT_EQ(pose_stamped.header.frame_id, transform_stamped.header.frame_id);
    EXPECT_EQ(pose_stamped.header.seq, transform_stamped.header.seq);
    EXPECT_DOUBLE_EQ(pose_stamped.header.stamp.toSec(), transform_stamped.header.stamp.toSec());
