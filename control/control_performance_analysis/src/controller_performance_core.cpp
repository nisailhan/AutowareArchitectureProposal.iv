...
ControlPerformanceCore::ControlPerformanceCore() : wheelbase_{2.74}
{
...
ControlPerformanceCore::ControlPerformanceCore(double wheelbase, double curvature_interval_length)
...
void ControlPerformanceCore::setCurrentWaypoints(
  const autoware_planning_msgs::Trajectory & trajectory)
{
  current_waypoints_ptr_ = std::make_shared<geometry_msgs::PoseArray>();
...
void ControlPerformanceCore::setCurrentPose(const geometry_msgs::Pose & msg)
{
  current_vec_pose_ptr_ = std::make_shared<geometry_msgs::Pose>(msg);
}

void ControlPerformanceCore::setCurrentControValue(
  const autoware_control_msgs::ControlCommandStamped & msg)
{
  current_control_ptr_ = std::make_shared<autoware_control_msgs::ControlCommandStamped>(msg);
}

std::pair<bool, int32_t> ControlPerformanceCore::findClosestPrevWayPointIdx_path_direction()
{
...
bool ControlPerformanceCore::isDataReady() const
{
  if (!current_vec_pose_ptr_) {
    ROS_WARN_THROTTLE(
      1.0,
      "[control_performance_analysis] cannot get current pose into control_performance algorithm");
    return false;
  }

  if (!current_waypoints_ptr_) {
    ROS_WARN_THROTTLE(
      1.0, "[control_performance_analysis] cannot get current trajectory waypoints ...");
    return false;
  }
  ...
  if (!current_control_ptr_) {
    ROS_WARN_THROTTLE(1.0, "[control_performance_analysis] waiting for current_steering ...");
    return false;
  }
...
std::pair<bool, TargetPerformanceMsgVars> ControlPerformanceCore::getPerformanceVars()
{
  ...
  std::pair<bool, geometry_msgs::Pose> pair_pose_interp_wp_ = calculateClosestPose();

  if (!pair_pose_interp_wp_.first || !interpolated_pose_ptr_) {
    ROS_WARN_THROTTLE(
      1.0,
      "[control_performance] Cannot get interpolated pose into control_performance "
      "algorithm");
  ...
  if (!idx_curve_ref_wp_) {
    ROS_ERROR("[control_performance_analysis] Cannot find index of curvature reference waypoint ");
    return std::make_pair(false, TargetPerformanceMsgVars{});
  }
  ...
  double && lateral_error = control_performance_utils::computeLateralError(
    interp_waypoint_xy, vehicle_position_xy, vehicle_yaw_angle);

  //Compute the yaw angle error.
  double && heading_yaw_error =
    control_performance_utils::angleDistance(vehicle_yaw_angle, target_yaw);
...
geometry_msgs::Pose ControlPerformanceCore::getPrevWPPose() const
{
  geometry_msgs::Pose pose_ref_waypoint_ = current_waypoints_ptr_->poses.at(*idx_prev_wp_);
  return pose_ref_waypoint_;
}
void ControlPerformanceCore::setCurrentVelocities(const geometry_msgs::Twist & twist_msg)
{
  current_velocities_ptr_->at(0) = twist_msg.linear.x;
  current_velocities_ptr_->at(1) = twist_msg.angular.z;
}
void ControlPerformanceCore::findCurveRefIdx()
{
  // Get the previous waypoint as the reference
  if (!interpolated_pose_ptr_) {
    ROS_WARN_THROTTLE(
      1.0, "[control_performance] Cannot set the curvature_idx, no valid interpolated pose ...");
...
std::pair<bool, geometry_msgs::Pose> ControlPerformanceCore::calculateClosestPose()
{
  geometry_msgs::Pose interpolated_pose;

  // Get index of prev waypoint and sanity check.
  if (!idx_prev_wp_) {
    ROS_ERROR("[kinematic_controller] Cannot find the next waypoint.");
  ...
  double dpsi_prev2next = control_performance_utils::angleDistance(next_yaw, prev_yaw);
  ...
  geometry_msgs::Quaternion orient_msg =
    control_performance_utils::createOrientationMsgfromYaw(interp_yaw_angle);
...
void ControlPerformanceCore::setInterpolatedPose(geometry_msgs::Pose & interpolated_pose)
{
  interpolated_pose_ptr_ = std::make_shared<geometry_msgs::Pose>(interpolated_pose);
}

double ControlPerformanceCore::estimateCurvature()
{
  // Get idx of front-axle center reference point on the trajectory.
  // get the waypoint corresponds to the front_axle center.
  geometry_msgs::Pose front_axleWP_pose = current_waypoints_ptr_->poses.at(*idx_curve_ref_wp_);
  ...
  geometry_msgs::Pose front_axleWP_pose_prev = current_waypoints_ptr_->poses.at(idx_prev_waypoint);
  ...
  // Define waypoints 10 meters behind the rear axle if exist. If not exist, we will take the first point of the
  ...
  // We have three indices of the three trajectory poses. We compute a curvature estimate from these points.
  ...
  double estimated_curvature =
    control_performance_utils::curvatureFromThreePoints(a_coord, b_coord, c_coord);

  return estimated_curvature;
}

double ControlPerformanceCore::estimatePurePursuitCurvature()
{
  if (!interpolated_pose_ptr_) {
    ROS_WARN_THROTTLE(
      1.0,
      "[control_performance_analysis] Cannot set pure pursuit_target_point_idx, no valid "
      "interpolated pose ...");
  ...
  geometry_msgs::Pose target_pose_pp;
  ...
  if (it == current_waypoints_ptr_->poses.cend()) {
    ...
    geometry_msgs::Pose const & last_pose_on_traj = current_waypoints_ptr_->poses.at(temp_idx_pp);
    ...
    std::vector<double> unit_tangent = control_performance_utils::getTangentVector(yaw_pp);
  ...
  std::vector<double> normal_vec =
    control_performance_utils::getNormalVector(current_vec_yaw);  // ClockWise
