...
SteerConverter::SteerConverter()
{
  ff_map_initialized_ = false;
  fb_gains_initialized_ = false;
  fb_limits_initialized_ = false;
}
  ...
  const double target_steer_angle_velocity, const double current_steer_angle) const
{
  if (!ff_map_initialized_) {
    ROS_WARN_THROTTLE(3.0, "FF map is not initialized!");
    return 0;
  }
  ...
  std::vector<double> & pid_contributions, std::vector<double> & errors)
{
  if (!fb_gains_initialized_ || !fb_limits_initialized_) {
    ROS_WARN_THROTTLE(3.0, "FB params are not initialized!");
    return 0;
  }
  ...
  std::vector<double> & output_index, std::vector<std::vector<double>> & steer_map) const
{
  ...
  if (!csv.readCSV(table)) {
    ROS_ERROR_THROTTLE(3.0, "[SteerConverter] Cannot open %s", csv_path.c_str());
    return false;
  }
  ...
  if (table[0].size() < 2) {
    ROS_ERROR_THROTTLE(
      3.0,
      "[SteerConverter] Cannot read %s. CSV file should have "
      "at least 2 column",
      csv_path.c_str());
    return false;
  }
  ...
  for (unsigned int i = 1; i < table.size(); ++i) {
    if (table[0].size() != table[i].size()) {
      ROS_ERROR_THROTTLE(
        3.0,
        "[SteerConverter] Cannot read %s. Each row should have "
...
void SteerConverter::calcFFMap(double steer_vel, double vehicle_vel, double & output) const
{
  ...
  if (vehicle_vel < vel_index_.front()) {
    ROS_WARN_THROTTLE(
      1.0,
      "[SteerConverter] Exceeding the vel range. Current vel: "
