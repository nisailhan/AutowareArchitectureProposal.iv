...
PacmodDynamicParameterChangerNode::PacmodDynamicParameterChangerNode() : nh_(), pnh_("~")
{
  pnh_.param<double>("straight_p_gain", straight_course_param_.p_gain, 0.8);
  pnh_.param<double>("straight_i_gain", straight_course_param_.i_gain, 0.002);
  pnh_.param<double>("straight_lugre_fc", straight_course_param_.lugre_fc, 0.03);
  pnh_.param<double>("straight_rtz_offset", straight_course_param_.rtz_offset, 0.085);
  pnh_.param<double>("curve_p_gain", curve_course_param_.p_gain, 0.3);
  pnh_.param<double>("curve_i_gain", curve_course_param_.i_gain, 0.002);
  pnh_.param<double>("curve_lugre_fc", curve_course_param_.lugre_fc, 0.01);
  pnh_.param<double>("curve_rtz_offset", curve_course_param_.rtz_offset, 0.085);
  pnh_.param<double>("min_steer_thresh", min_steer_thresh_, 0.2);
  pnh_.param<double>("max_steer_thresh", max_steer_thresh_, 0.5);
  pnh_.param<double>("p_gain_rate_max", param_max_rate_.p_gain, 5.0);
  pnh_.param<double>("i_gain_rate_max", param_max_rate_.i_gain, 0.05);
  pnh_.param<double>("lugre_fc_rate_max", param_max_rate_.lugre_fc, 0.1);
  pnh_.param<double>("rtz_offset_rate_max", param_max_rate_.rtz_offset, 0.5);
  pnh_.param<double>("p_gain_rate_min", param_min_rate_.p_gain, -1.5);
  pnh_.param<double>("i_gain_rate_min", param_min_rate_.i_gain, -0.05);
  pnh_.param<double>("lugre_fc_rate_min", param_min_rate_.lugre_fc, -0.1);
  pnh_.param<double>("rtz_offset_rate_min", param_min_rate_.rtz_offset, -0.5);

  current_param_list_ = curve_course_param_;

  can_pub_ = pnh_.advertise<can_msgs::Frame>("output/can", 1, true);
  debug_pub_ = pnh_.advertise<std_msgs::Float32MultiArray>("debug/parameter", 1);
  steer_rpt_sub_ =
    pnh_.subscribe("input/steer_rpt", 1, &PacmodDynamicParameterChangerNode::subSteerRpt, this);
}

void PacmodDynamicParameterChangerNode::subSteerRpt(const pacmod_msgs::SystemRptFloat::ConstPtr msg)
{
  current_param_list_ = calculateParam(std::abs(msg->command), std::abs(msg->output), msg->enabled);
  current_param_time_ = ros::Time::now();
  ...
  const ParamList new_param, const ParamList current_param)
{
  ...
  const double dt = (ros::Time::now() - current_param_time_).toSec();
