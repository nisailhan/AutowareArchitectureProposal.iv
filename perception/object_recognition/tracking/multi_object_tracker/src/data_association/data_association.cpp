...
Eigen::Matrix2d getXYCovariance(const geometry_msgs::PoseWithCovariance & pose_covariance)
{
...
double getFormedYawAngle(
  const geometry_msgs::Quaternion & measurement_quat,
  const geometry_msgs::Quaternion & tracker_quat, const bool distinguish_front_or_back = true)
{
  ...
  const std::list<std::shared_ptr<Tracker>> & trackers)
{
  ...
  for (auto tracker_itr = trackers.begin(); tracker_itr != trackers.end();
    ...
    for (size_t measurement_idx = 0; measurement_idx < measurements.feature_objects.size();
         ++measurement_idx) {
      double score = 0.0;
      const geometry_msgs::PoseWithCovariance tracker_pose_covariance =
        (*tracker_itr)->getPoseWithCovariance(measurements.header.stamp);
      const autoware_perception_msgs::DynamicObject & measuement_object =
        measurements.feature_objects.at(measurement_idx).object;
      if (can_assign_matrix_((*tracker_itr)->getType(), measuement_object.semantic.type)) {
        const double max_dist =
          max_dist_matrix_((*tracker_itr)->getType(), measuement_object.semantic.type);
        const double max_area =
          max_area_matrix_((*tracker_itr)->getType(), measuement_object.semantic.type);
        const double min_area =
          min_area_matrix_((*tracker_itr)->getType(), measuement_object.semantic.type);
        const double max_rad =
          max_rad_matrix_((*tracker_itr)->getType(), measuement_object.semantic.type);
        const double dist = getDistance(
          measuement_object.state.pose_covariance.pose.position,
          tracker_pose_covariance.pose.position);
        const double area = utils::getArea(measuement_object.shape);
        ...
        // angle gate
          ...
          const double angle = getFormedYawAngle(
            measuement_object.state.pose_covariance.pose.orientation,
