...
bool NormalVehicleTracker::predict(const ros::Time & time)
{
...
bool NormalVehicleTracker::measureWithPose(const autoware_perception_msgs::DynamicObject & object)
{
  float r_cov_x;
  float r_cov_y;
  ...
  if (!ekf_.update(Y, C, R)) ROS_WARN("Vehicle : Cannot update");
...
bool NormalVehicleTracker::measureWithShape(const autoware_perception_msgs::DynamicObject & object)
{
  ...
  const autoware_perception_msgs::DynamicObject & object, const ros::Time & time)
{
  ...
  if (0.01 /*10msec*/ < std::fabs((time - last_update_time_).toSec())) {
    ROS_WARN(
      "Vehicle : There is a large gap between predicted time and measurement time. (%f)",
