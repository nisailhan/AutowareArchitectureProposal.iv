...
bool PedestrianTracker::predict(const double dt, KalmanFilter & ekf)
{
  ...
  if (!ekf_.predict(X_next_t, A, Q)) ROS_WARN("Pedestrian : Cannot predict");

  return true;
}

bool PedestrianTracker::measureWithPose(const autoware_perception_msgs::DynamicObject & object)
{
  ...
  if (!ekf_.update(Y, C, R)) ROS_WARN("Pedestrian : Cannot update");
...
bool PedestrianTracker::measureWithShape(const autoware_perception_msgs::DynamicObject & object)
{
