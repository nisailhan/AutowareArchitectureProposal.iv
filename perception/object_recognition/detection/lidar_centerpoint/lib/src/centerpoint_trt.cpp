...
bool CenterPointTRT::initPtr(const bool use_vfe_trt, const bool use_head_trt)
{
  ...
  if (use_head_trt) {
    const int downsample_grid_x = (int)((float)Config::grid_size_x / Config::downsample_factor);
    const int downsample_grid_y = (int)((float)Config::grid_size_y / Config::downsample_factor);
...
std::vector<float> CenterPointTRT::detect(const sensor_msgs::PointCloud2 & input_pointcloud_msg)
{
  ...
  int num_voxels = vg_ptr_->pointsToVoxels(input_pointcloud_msg, voxels_t_, coordinates_t_, num_points_per_voxel_t_);
  // Note: unlike python implementation, no slicing by num_voxels .s.t voxels_t_ = voxels_t_[:num_voxels].
