...
sensor_msgs::PointCloud2 PointCloudDensification::stackPointCloud(
  const sensor_msgs::PointCloud2 & input_pointcloud_msg)
{
  const double input_timestamp = input_pointcloud_msg.header.stamp.toSec();
  ...
  for (; pc_msg_iter != pointcloud_cache_.end(); pc_msg_iter++, matrix_iter++) {
    sensor_msgs::PointCloud2 cached_pointcloud_msg = *pc_msg_iter;
    ...
    double diff_timestamp = input_timestamp - cached_pointcloud_msg.header.stamp.toSec();
    setTimeLag(transformed_pointcloud_msg, (float)diff_timestamp);

    sensor_msgs::PointCloud2 tmp_pointcloud_msg = output_pointcloud_msg;
...
geometry_msgs::TransformStamped PointCloudDensification::getTransformStamped(
...
void PointCloudDensification::setTimeLag(
  sensor_msgs::PointCloud2 & pointcloud_msg, const float diff_timestamp)
{
  ...
  for (int i = 0; i < width; i++) {
    memcpy(&pointcloud_msg.data[offset + point_step * i], (void *)&diff_timestamp, sizeof(float));
  }
}
